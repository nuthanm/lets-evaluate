...
def generate_evaluation_pdf(evaluation_data: dict) -> bytes:
    ...
    if metrics:
        ...
        matched = metrics.get("matched_technologies", [])
        missing = metrics.get("missing_technologies", [])

        sc_data = [[
            Paragraph("<b>✅ Strengths</b>", styles["small"]),
            Paragraph("<b>⚠️ Concerns</b>", styles["small"]),
            Paragraph("<b>✔ Matched Tech</b>", styles["small"]),
            Paragraph("<b>✘ Missing Tech</b>", styles["small"]),
        ], [
            Paragraph("<br/>".join(f"• {s}" for s in strengths) or "—", styles["small"]),
            Paragraph("<br/>".join(f"• {c}" for c in concerns) or "—", styles["small"]),
            Paragraph("<br/>".join(f"• {t}" for t in matched) or "—", styles["small"]),
            Paragraph("<br/>".join(f"• {t}" for t in missing) or "—", styles["small"]),
        ]]
        sc_table = Table(sc_data, colWidths=[page_w * 0.25] * 4)
